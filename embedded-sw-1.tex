...
\begin{document}
    \begin{layout}
        ...
        % \input{sections/01_embsys.tex}
        % % Modellierung von Embedded (real-time) Systems (Model Driven Development, MDD)
        % \input{sections/02_rtsys.tex}
        % \input{sections/03_modeling.tex}
        % % Hardware/Software Codesign
        % \input{sections/04_HW-SW-codesign.tex}
        % % Finite State Machines (Anwendung, Implementationen in C und C++)
        % \input{sections/05_fsm.tex}
        % \input{sections/06_statecharts.tex}
        % \input{sections/07_fsm_realization_flat.tex}
        % \input{sections/08_modularization.tex}
        % % Embedded Software Patterns
        % \input{sections/09_patterns.tex}
        % % Event-based Systems
        % \input{sections/10_event_based_systems.tex}
        % \input{sections/11_scheduling.tex}
        % % Gleichzeitigkeit (Concurrency)
        % \input{sections/12_concurrency.tex}
        % % POSIX-Programmierung
        % \input{sections/13_posix_threads.tex}
        % \input{sections/14_RAII.tex}
        % \input{sections/15_interfacing_C.tex}
        % \input{sections/16_programming_style_guide.tex}
        % % Multicore Systems mit Multistage-Caches, Interprozesskommunikation, Shared Memory
        % \input{sections/17_multicore_systeme.tex}
        % % Real-time Operating System (RTOS)
        % \input{sections/18_RTOS.tex}
        % Hardware Abstraction Layer (HAL) in C und C++
        \input{sections/19_HAL.tex}
        \input{sections/20_inline_makros.tex}
    \end{layout}
\end{document}
